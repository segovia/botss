...
{
	A.push(contact->GetFixtureA()->GetBody()->GetUserDataInt());
	B.push(contact->GetFixtureB()->GetBody()->GetUserDataInt());
}
...
b2World::b2World(const b2Vec2& gravity)
{
	...
	m_allowSleep = true;
	m_gravity = gravity;
	m_useGravityCenter = false;
	m_gravityCenter = b2Vec2(0.0f, 0.0f);
...
void b2World::AddMyContactListener()
{
	myContactListener = new MyContactListener();
	SetContactListener(myContactListener);
}

void b2World::PopulateBeginContacts(int32 AB[])
{
	for (int32 i = 0; i < myContactListener->lastSize; ++i)
	{
		AB[2*i] = myContactListener->A.front();
		AB[2*i+1] = myContactListener->B.front();
		myContactListener->A.pop();
		myContactListener->B.pop();
	}
}

int32 b2World::GetBeginContactsArraySize()
{
	myContactListener->lastSize = myContactListener->A.size();
	return myContactListener->lastSize * 2;
}
...
void b2World::Solve(const b2TimeStep& step)
{
	...
	for (b2Body* seed = m_bodyList; seed; seed = seed->m_next)
	{
		...
		b2Profile profile;
		island.Solve(&profile, step, m_gravity, m_allowSleep, m_useGravityCenter, m_gravityCenter);
