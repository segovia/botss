...
b2ParticleSystem::~b2ParticleSystem()
{
	...
	FreeUserOverridableBuffer(&m_positionAndVelocityBuffer);
...
void b2ParticleSystem::ReallocateInternalAllocatedBuffers(int32 capacity)
{
	...
	if (m_internalAllocatedCapacity < capacity)
	{
		...
		m_positionAndVelocityBuffer.data = ReallocateBuffer(
			&m_positionAndVelocityBuffer, m_internalAllocatedCapacity*2, capacity*2, false);
...
void b2ParticleSystem::UpdateBodyContacts()
{
	...
	{
		const int32 particleCount = GetParticleCount();
		int32 blockSize = ceil(particleCount / 8.0f);
		int32 blocks = ceil(float(particleCount) / blockSize);
		#pragma omp parallel for
		for(int32 parallel_i = 0; parallel_i < blocks; ++parallel_i) {
			int32 max = parallel_i == blocks - 1 ? particleCount : (parallel_i+1) * blockSize;
			for (int32 i = parallel_i * blockSize; i < max; i++)
			{
	...
	class UpdateBodyContactsCallback : public b2QueryCallback
	{
		...
		bool ReportFixture(b2Fixture* fixture)
		{
			...
			if ((fixture->GetFilterData().maskBits & 1) == 0) return true;
...
void b2ParticleSystem::Solve(const b2TimeStep& step)
{
		...
		m_iterationIndex++)
	{
		...
		for (int32 i = 0; i < m_count; i++)
		{
			...
			m_positionAndVelocityBuffer.data[2*i] = m_positionBuffer.data[i];
			m_positionAndVelocityBuffer.data[2*i+1] = m_velocityBuffer.data[i];
		}

	}
}

int32 b2ParticleSystem::GetUniqueBodyContactCount(int32 minParticles) {
	std::map<int32, int32> bodyMap;

	for (int32 k = 0; k < m_bodyContactCount; k++)
	{
		int32 bodyId = m_bodyContactBuffer[k].body->GetUserDataInt();
		std::map<int32, int32>::iterator it = bodyMap.find(bodyId);
		if (it != bodyMap.end()) {
			it->second += 1;
		} else {
			bodyMap.insert(std::pair<int32, int32>(m_bodyContactBuffer[k].body->GetUserDataInt(), 1));
		}
	}

	m_uniqueBodyContactBuffer.clear();
	for (std::map<int32, int32>::iterator it = bodyMap.begin(); it != bodyMap.end(); ++it) {
		if (it->second >= minParticles) {
			m_uniqueBodyContactBuffer.push_back(it->first);
		}
	}
	return m_uniqueBodyContactBuffer.size();
}

void b2ParticleSystem::PopulateUniqueBodyContacts(int32 contacts[]) {

	for (int32 k = 0; k < m_uniqueBodyContactBuffer.size(); k++)
	{
		contacts[k] = m_uniqueBodyContactBuffer[k];
	}
}
...
void b2ParticleSystem::LimitVelocity(const b2TimeStep& step)
{
	...
	int32 blockSize = ceil(m_count / 8.0f);
	int32 blocks = ceil(float(m_count) / blockSize);
	#pragma omp parallel for
	for(int32 parallel_i = 0; parallel_i < blocks; ++parallel_i) {
		int32 max = parallel_i == blocks - 1 ? m_count : (parallel_i+1) * blockSize;
		for (int32 i = parallel_i * blockSize; i < max; i++)
		{
			b2Vec2& v = m_velocityBuffer.data[i];
...
void b2ParticleSystem::SolveGravity(const b2TimeStep& step)
{
	...
	if (m_world->UseGravityCenter())
	{
		float32 gravityStrength = step.dt * m_gravityScale * m_world->GetGravity().Length();
		b2Vec2 gravityCenter = m_world->GetGravityCenter();
		...
		for(int32 parallel_i = 0; parallel_i < blocks; ++parallel_i) {
			...
			for (int32 i = parallel_i * blockSize; i < max; i++)
			{
				b2Vec2 gravityVec = gravityCenter - m_positionBuffer.data[i];
				gravityVec.Normalize();
				m_velocityBuffer.data[i] += gravityVec * gravityStrength;
			}
		}
